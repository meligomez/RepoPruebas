...
namespace PalcoNet.Editar_Publicacion
{
	public partial class EditarPublicacion : Form
	{
		...
		private void EditarPublicacion_Load(object sender, EventArgs e)
		{
			...
			string query = "SELECT p.id as 'Codigo',r.Descripcion as 'Rubro',g.tipo as 'Grado',p.descripcion as 'Descr. Espectaculo',stock,fechaPublicacion as 'Fecha Publicacion',fechaEspectaculo as 'Fecha Espectaculo',direccion as 'Direccion Espec.'FROM dropeadores.Publicacion p "+
				" join dropeadores.Rubro r on(r.Codigo=p.rubroId)" +
				" join dropeadores.Grado g on(g.id=p.gradoId)"+
				" where empresaId= '" + userLogueado.empresa.Empresa_Cuit + "' and estado=0";
